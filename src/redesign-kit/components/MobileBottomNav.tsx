/**
 * Mobile bottom navigation — Material 3 pattern com overflow sheet.
 *
 * Requisitos no repo destino:
 *   - react-router-dom (NavLink, useNavigate)
 *   - lucide-react
 *   - Função opcional `hapticSelection()` e `hapticImpact()` (do Capacitor Haptics).
 *     Se não existirem, troca por no-op ou remove as chamadas.
 *
 * IMPORTANTE: edita os arrays `primaryItems` e `overflowItems` pras rotas do seu app.
 */
...
// EDITA pras rotas do seu app — 4 destinos primários
...
// EDITA pras rotas secundárias (vão dentro do sheet "Mais")
const overflowItems: BottomNavItem[] = [
  ...
  { to: '/promocoes',         icon: Sparkles,     label: 'Promoções' },
  { to: '/titulares',         icon: UserIcon,     label: 'Titulares' },
  { to: '/agencia/dashboard', icon: Building2,    label: 'Agência' },
